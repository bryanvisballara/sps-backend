...
import { clearNotificationLog, notifyExpiryThreshold, notifyInventoryAlert, sendDedupedNotification, } from "./push-notification.service.js";
const DAY_MS = 24 * 60 * 60 * 1000;
const MONITOR_INTERVAL_MS = 60 * 60 * 1000;
function daysUntil(date, now) {
    return Math.ceil((date.getTime() - now.getTime()) / DAY_MS);
}
function expiryLogKey(thresholdDays, productId, stockRowId) {
    return `expiry-${thresholdDays}-${productId}-${stockRowId}`;
}
function inventoryLogKey(productId, stockRowId) {
...
async function checkInventoryAlerts() {
    ...
    for (const product of products) {
        ...
        const targetStockRowId = productStockRows.length > 0
            ...
            : "product";
        await sendDedupedNotification(inventoryLogKey(productId, targetStockRowId), "inventory-alert", async () => {
            await notifyInventoryAlert(product.name, product.sku, totalQuantity, alertLevel);
        });
    }
}
async function checkExpiryAlerts(now) {
    const [products, stockRows] = await Promise.all([
    ...
    for (const lot of lots) {
        ...
        for (const threshold of thresholds) {
            ...
            await sendDedupedNotification(expiryLogKey(threshold, lot.productId, lot.stockRowId), `expiry-${threshold}`, async () => {
...
//# sourceMappingURL=inventory-push-monitor.service.js.map