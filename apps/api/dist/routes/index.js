...
function normalizeImportBoolean(value, fallback = true) {
...
function getImportRowField(row, candidates) {
...
async function ensureImportSupplier(name) {
    const trimmedName = name.trim();
    if (!trimmedName) {
...
function parseCloudinaryDeliveryUrl(value) {
    const trimmed = String(value ?? "").trim();
    const match = trimmed.match(/^https?:\/\/res\.cloudinary\.com\/([^/]+)\/(image|raw|video|auto)\/(upload|authenticated|private)\/(?:v\d+\/)?(.+)$/i);
    if (!match) {
        return null;
    }
...
function normalizeSalesRoutePayload(body) {
...
function normalizeWarehouseLocationPayload(body) {
    if (typeof body !== "object" || body === null) {
...
function getCloudinaryFolder(purpose) {
...
function summarizeImportExpenses(expenseItems) {
    return expenseItems.reduce((summary, expense) => {
        switch (expense.key) {
            case "freight":
...
function normalizeCatalogWhatsappPayload(body) {
    if (typeof body !== "object" || body === null) {
...
async function normalizeClientPayload(body) {
    if (typeof body !== "object" || body === null) {
...
function normalizeImportCostPayload(body) {
    ...
    const products = rawProducts.map((productEntry, index) => {
        ...
        const currentProduct = productEntry;
...
async function buildImportCostRows(payload, containerReference) {
    ...
    return baseRows.map((row) => {
        ...
        const totalImportCost = row.merchandiseCostTotal + freightCost + customsCost + inlandLogisticsCost + taxesCost + otherImportCosts;
...
function normalizeFixedCostPayload(body) {
    if (typeof body !== "object" || body === null) {
...
function normalizeOperationalExpensePayload(body) {
    if (typeof body !== "object" || body === null) {
...
function addDaysToBusinessDateKey(dateKey, days) {
    const parsed = new Date(`${dateKey}T12:00:00`);
...
function normalizeOrderGiftItems(rawItems) {
...
async function createSubmittedSalesOrder(payload) {
    ...
    if (!store) {
        throw Object.assign(new Error("El cliente no existe."), { statusCode: 404 });
    }
    if (!salesRep || salesRep.role !== "sales-rep-aruba") {
        throw Object.assign(new Error("El vendedor no existe o no esta habilitado."), { statusCode: 404 });
    }
    ...
    const availableProductIds = new Set(productsById.keys());
...
async function mapWarehouseOrderRecord(order) {
    ...
    return {
        ...
        invoiceVoidedByUserName: String(order.invoiceVoidedByUserName ?? ""),
        invoiceVoidedByRole: String(order.invoiceVoidedByRole ?? ""),
        ...
        items: order.items.map((item) => {
            ...
            const relatedProduct = productsById.get(productId);
            const frozenSalePrice = Number(item.salePriceAwg ?? NaN);
            const catalogSalePrice = catalogSalePriceByProductId.get(productId);
            const hasFrozenSalePrice = Number.isFinite(frozenSalePrice) && frozenSalePrice >= 0;
            const preferCatalogForOpenOrder = catalogSalePrice !== undefined && order.status !== "delivered";
            const resolvedSalePrice = preferCatalogForOpenOrder
                ? (hasFrozenSalePrice && frozenSalePrice <= catalogSalePrice + 0.009
                    ? Math.round(frozenSalePrice * 100) / 100
                    : catalogSalePrice)
                : hasFrozenSalePrice
                    ? Math.round(frozenSalePrice * 100) / 100
                    : catalogSalePrice;
...
function sendCreationError(response, error) {
...
function canEditSellerOrder(createdAt) {
...
/**
 * Returns productId -> salePrice for a client from CatalogClientPricing.
 * If the client is in several catalogs, the most recently updated pricing wins per product.
 */
async function getClientCatalogProductSalePrices(clientId) {
    ...
    pricingRows.forEach((pricingRow) => {
        if (!Array.isArray(pricingRow.items)) {
            return;
        }
...
function resolveClientAwareSalePrice(params) {
    const catalogSalePrice = params.catalogSalePriceByProductId?.get(params.productId);
    if (catalogSalePrice !== undefined) {
        return catalogSalePrice;
    }
    const lotSalePrice = Number(params.lotSalePriceAwg ?? 0);
    if (lotSalePrice > 0) {
        return Math.round(lotSalePrice * 100) / 100;
    }
    return Math.round(Math.max(0, Number(params.productSalePrice ?? 0)) * 100) / 100;
}
...
async function getSalesRepRouteStoreIds(salesRepId) {
    const routes = await SalesRoute.find({ salesRepId, active: { $ne: false } }).lean();
    return new Set(routes.flatMap((route) => route.days.flatMap((day) => day.stores.map((store) => store.storeId))));
}
async function assertSalesRepCanManageStore(salesRepId, storeId) {
...
async function buildSalesProductCatalog(storeId) {
    const [products, stockByProduct, store, catalogSalePriceByProductId] = await Promise.all([
        ...
        storeId ? getClientCatalogProductSalePrices(storeId) : Promise.resolve(new Map()),
    ]);
    ...
    const rows = products.map((product) => {
        ...
        const baseSalePrice = resolveClientAwareSalePrice({
            productId,
            productSalePrice: product.salePrice,
            lotSalePriceAwg: stockInfo.preferredLot?.salePriceAwg,
            catalogSalePriceByProductId,
        });
        const hasCatalogPrice = catalogSalePriceByProductId.has(productId);
        ...
        const isExpiringSoon = Boolean(nearestExpiration &&
        ...
        return {
            ...
            priceSource: hasCatalogPrice ? "catalog" : promotion ? "promotion" : "product",
            promotion,
...
async function sanitizeSalesRoutesWithStores(routes) {
    const typedRoutes = routes;
    const storeIds = [...new Set(typedRoutes.flatMap((route) => route.days.flatMap((day) => day.stores.map((store) => store.storeId))))].filter(Boolean);
...
async function removeStoreFromAllRoutes(storeId) {
    ...
    await Promise.all(routes.map(async (route) => {
        ...
        await SalesRoute.updateOne({ _id: route._id }, { $set: { days: sanitizedDays, plannedStops, assignedDays } });
    }));
}
function compareWarehouseStockLotsByConsumptionPriority(left, right) {
...
function buildOrderQuantitiesMap(items) {
...
async function restoreInventoryForProduct(productId, quantityToRestore, orderId) {
    ...
    if (stockRows.length > 0) {
        ...
        await WarehouseStock.findByIdAndUpdate(targetRow._id, {
            availableUnits: nextAvailable,
...
async function deductAdditionalInventoryForProduct(productId, quantityToDeduct, orderId) {
...
async function applyOrderInventoryDelta(orderId, oldItems, newItems) {
...
function normalizeInvoiceChangeRequestItems(rawItems, existingItems) {
...
async function calculateInvoiceAmountFromItems(items) {
...
apiRouter.post("/sales/stores/:id/assigned-products/add", async (request, response) => {
    try {
        ...
        store.assignedProductIds = [...currentAssignedProductIds, productId];
        await store.save();
        response.status(201).json({
...
apiRouter.get("/sales/stores/:id/products", async (request, response) => {
    try {
        ...
        const products = await Product.find({ _id: { $in: assignedProductIds }, active: { $ne: false }, shareWithAruba: { $ne: false } })
            .sort({ name: 1 })
            .lean();
        const [stockByProduct, catalogSalePriceByProductId] = await Promise.all([
            aggregateWarehouseStockByProduct(),
            getClientCatalogProductSalePrices(String(store._id)),
        ]);
        response.json({
            store: {
                id: String(store._id),
...
apiRouter.post("/sales/orders", async (request, response) => {
    try {
        ...
        const order = await createSubmittedSalesOrder(payload);
        response.status(201).json({
            message: `Pedido enviado a bodega para ${order.storeName}.`,
            order,
            invoiceNumber: Number(order.invoiceNumber ?? 0) || null,
        });
...
apiRouter.post("/management/orders/direct-invoice", async (request, response) => {
    try {
        ...
        const internalOrderNotes = payload.internalOrderNotes
            ...
            : directNote;
        const order = await createSubmittedSalesOrder({
            ...payload,
            internalOrderNotes,
        });
        const completeResult = await completeWarehouseOrderById(String(order._id), {
            ...
            editedByUserName: body.editedByUserName,
            editedByRole: body.editedByRole,
        });
        response.status(201).json({
            message: completeResult.message.includes("facturado")
                ? `Pedido facturado de inmediato para ${order.storeName} con consecutivo #${completeResult.invoiceNumber}.`
                : completeResult.message,
            order: {
                _id: String(order._id),
                status: completeResult.order.status,
                storeName: order.storeName,
                updatedAt: completeResult.order.updatedAt,
            },
            invoiceNumber: completeResult.invoiceNumber,
            carteraEntryId: completeResult.carteraEntryId,
        });
    }
    catch (error) {
        if (error instanceof Error && "statusCode" in error && typeof error.statusCode === "number") {
...
apiRouter.put("/sales/orders/:id", async (request, response) => {
    try {
        ...
        if (order.salesRepId !== payload.salesRepId ||
        ...
        const catalogSalePriceByProductId = await getClientCatalogProductSalePrices(payload.storeId);
        ...
        const actor = normalizeOrderEditActor(typeof request.body === "object" && request.body !== null ? request.body : {}, {
...
apiRouter.put("/warehouse/orders/:id/invoice-number", async (request, response) => {
    try {
        ...
        if (order.invoiceVoided === true) {
            response.status(400).json({ message: "No puedes cambiar el consecutivo de una factura anulada." });
            return;
        }
...
function buildBusinessMonthMongoFilter(dateField, monthFilter) {
...
function getCarteraInvoiceAgeDays(invoicedAt, referenceDate = new Date()) {
...
function resolveOrderEditSource(role) {
...
function normalizeOrderEditActor(body, fallback) {
    const userId = String((typeof body?.editedByUserId === "string" && body.editedByUserId.trim())
        || (typeof body?.requestedByUserId === "string" && body.requestedByUserId.trim())
        || fallback?.userId
        || "").trim();
    const userName = String((typeof body?.editedByUserName === "string" && body.editedByUserName.trim())
        || (typeof body?.requestedByUserName === "string" && body.requestedByUserName.trim())
        || fallback?.userName
        || "").trim();
    const role = String((typeof body?.editedByRole === "string" && body.editedByRole.trim())
...
async function buildOrderItemsEditChanges(previousItems, nextItems, fieldLabel) {
    const productIds = [
...
function resolveFrozenOrderItemSalePrice(item, productSalePrice = 0, catalogSalePrice) {
    ...
    if (catalogSalePrice !== undefined && Number.isFinite(catalogSalePrice) && catalogSalePrice >= 0) {
...
async function buildWarehouseInvoiceDocumentLines(order) {
    ...
    const [products, catalogSalePriceByProductId] = await Promise.all([
        productIds.length > 0
            ? Product.find({ _id: { $in: productIds } }).select({ _id: 1, name: 1, sku: 1, description: 1, salePrice: 1 }).lean()
            : Promise.resolve([]),
        storeId ? getClientCatalogProductSalePrices(storeId) : Promise.resolve(new Map()),
    ]);
    const productsById = new Map(products.map((product) => [String(product._id), product]));
    const regularLines = orderItems.flatMap((item) => {
        ...
        const rate = resolveFrozenOrderItemSalePrice(item, Number(product?.salePrice ?? 0), catalogSalePriceByProductId.get(productId));
...
async function syncLogisticsInvoiceItemsFromOrder(order, invoiceLines) {
    ...
    const previousCostByProductId = new Map((Array.isArray(existingInvoice.items) ? existingInvoice.items : []).map((item) => [
    ...
    const totalCostAwg = Math.round(items.reduce((sum, item) => sum + Number(item.unitCostAwg ?? 0) * Number(item.quantity ?? 0), 0) * 100) / 100;
    await LogisticsInvoice.findByIdAndUpdate(existingInvoice._id, {
...
apiRouter.get("/warehouse/orders/:id/invoice-document", async (request, response) => {
    try {
        ...
        const linesTotalAmount = Math.round(items.reduce((sum, item) => sum + Number(item.amount ?? 0), 0) * 100) / 100;
...
async function voidDeliveredInvoiceById(orderId, body = {}) {
    ...
    if (order.status !== "delivered") {
        throw Object.assign(new Error("Solo puedes anular facturas de pedidos completados."), { statusCode: 400 });
    }
    if (order.invoiceVoided === true) {
        throw Object.assign(new Error("Esta factura ya esta anulada."), { statusCode: 400 });
    }
    const actor = normalizeOrderEditActor(body);
    ...
    await recordOrderEditLog({
        ...
        changes: [{
                ...
                summary: invoiceNumber
                    ? `Factura #${invoiceNumber} anulada${voidReason ? `: ${voidReason}` : ""}`
                    : `Factura anulada${voidReason ? `: ${voidReason}` : ""}`,
                before: "activa",
                after: "anulada",
            }],
    });
...
async function deleteWarehouseOrderWithCleanup(orderId) {
    const order = await Order.findById(orderId).lean();
    if (!order) {
    ...
    await deactivateCarteraForOrder(String(order._id));
    await Order.findByIdAndDelete(order._id);
    await OrderDeleteRequest.updateMany({ orderId: String(order._id), status: "pending" }, {
        status: "rejected",
...
async function completeWarehouseOrderById(orderId, body = {}) {
    ...
    if (order.status === "delivered") {
        const actor = normalizeOrderEditActor(body);
        if (order.invoiceVoided === true) {
            throw Object.assign(new Error("Esta factura ya esta anulada. No se puede volver a facturar."), { statusCode: 400 });
        }
        if (actor) {
            await recordOrderEditLog({
                orderId: String(order._id),
...
async function cancelWarehouseOrderDispatch(orderId) {
    const order = await Order.findById(orderId).lean();
    if (!order) {
        throw new Error("El pedido no existe.");
    }
...
apiRouter.put("/warehouse/orders/:id/void-invoice", async (request, response) => {
    try {
        const body = typeof request.body === "object" && request.body !== null
            ? request.body
            : {};
...
apiRouter.post("/warehouse/orders/:id/invoice-change-requests", async (request, response) => {
    try {
        ...
        const currentItems = await enrichInvoiceChangeItems(order.items.map((item) => ({
            productId: String(item.productId),
            quantity: Number(item.quantity ?? 0),
        ...
        const createdRequest = await InvoiceChangeRequest.create({
            ...
            currentPaymentMethod: String(carteraEntry.paymentMethod ?? "credito"),
        });
        response.status(201).json({
...
apiRouter.get("/management/stores/:id/summary", async (request, response) => {
    try {
        ...
        const [assignedProducts, routes, orders, billedInvoices] = await Promise.all([
            assignedProductIds.length > 0
                ? Product.find({ _id: { $in: assignedProductIds }, active: { $ne: false }, shareWithAruba: { $ne: false } })
                    ...
                    .lean()
                : Promise.resolve([]),
            sanitizeSalesRoutesWithStores(await SalesRoute.find({ active: { $ne: false } }).sort({ weekStart: -1, createdAt: -1 }).lean()),
        ...
        const orderProductIds = Array.from(new Set(orders.flatMap((order) => order.items.map((item) => String(item.productId)).filter(Boolean))));
        ...
        const totalUnitsOrdered = enrichedOrders.reduce((total, order) => total + order.items.reduce((sum, item) => sum + Number(item.quantity ?? 0), 0), 0);
        response.json({
            store: {
                id: String(store._id),
                code: store.code ?? "",
...
async function buildProductPerformanceRankings() {
    ...
    const expiringLowSales = rankedProducts
        ...
        return Boolean(inventory?.nearestExpiration
...
function invoiceDateInRange(invoiceDate, rangeStart, rangeEnd) {
...
function sumInvoiceRevenueInRange(invoices, rangeStart, rangeEnd) {
    return invoices.reduce((total, invoice) => (invoiceDateInRange(invoice.invoiceDate, rangeStart, rangeEnd)
        ? total + Number(invoice.totalRevenueAwg ?? 0)
        : total), 0);
}
function roundPerformanceGoalAwg(value) {
    if (!Number.isFinite(value) || value <= 0) {
        return 5000;
    }
    return Math.ceil(value / 500) * 500;
}
function suggestWeeklyGoalFromInvoices(invoices) {
...
function buildOrderSubmissionTimeline(orders) {
    const sortedAsc = [...orders].sort((left, right) => (new Date(String(left.createdAt ?? "")).getTime() - new Date(String(right.createdAt ?? "")).getTime()));
...
function normalizeSalesRepGoalPayload(body) {
    const payload = body;
    return {
...
async function buildSalesRepPerformanceBundle() {
    ...
    const enrichedOrders = await enrichOrdersForPerformance(orders);
    const performance = salesReps.map((rep) => {
        const salesRepId = String(rep._id);
        const salesRepName = String(rep.name ?? rep.email ?? "Vendedor");
        const repRoutes = routes.filter((route) => String(route.salesRepId ?? "") === salesRepId);
        ...
        const monthlyProgress = hasCustomGoals && monthlyGoalAwg > 0
            ...
            : 0;
        const assignedStoresMap = new Map();
        ...
        const ordersByRoute = Array.from(repOrders.reduce((map, order) => {
            const key = order.routeName || "Sin ruta";
            const current = map.get(key) ?? { routeName: key, orderCount: 0, storeNames: new Set() };
            current.orderCount += 1;
        ...
        const ordersByStore = Array.from(repOrders.reduce((map, order) => {
            ...
            return map;
        }, new Map()).values()).sort((left, right) => right.orderCount - left.orderCount);
        return {
            salesRepId,
            salesRepName,
...
apiRouter.put("/management/performance/sales-reps/:id/goals", async (request, response) => {
    try {
        ...
        const goal = await SalesRepGoal.findOneAndUpdate({ salesRepId }, {
            salesRepId,
...
apiRouter.get("/management/inventory-summary", async (request, response) => {
    ...
    const rows = products.flatMap((product) => {
        ...
        const importedQuantity = productImportRows.reduce((sum, row) => sum + Number(row.importedQuantity ?? 0), 0);
        ...
        const expenseCategoryTotals = (latestImportRow?.expenseItems ?? []).reduce((sum, expense) => {
            ...
            ? {
                ...
                expenseRows: (latestImportRow.expenseItems ?? []).map((expense, index) => {
                    ...
                    const allocatedTotal = categoryTotal > 0
                        ? categorizedAllocatedTotals[key] * (expenseAmount / categoryTotal)
                        : 0;
                    return {
    ...
    const kpis = {
        ...
        totalInventoryCost: rows.reduce((sum, row) => sum + row.totalCost, 0),
    ...
    try {
        ...
        for (const item of items) {
            ...
            await InventoryAdjustment.create({
                ...
                notes: entryNotes || "Registro de entrada manual o por carga de Excel.",
                entryGroupId,
...
async function handleDeleteInventoryEntryGroup(request, response) {
    try {
        ...
        const stockRowsByRemovalKey = new Map();
...
apiRouter.put("/management/inventory-entries/:groupId", async (request, response) => {
    try {
        ...
        const usdToAwgRate = Number(request.body?.usdToAwgRate ?? 0);
        const entryDateRaw = typeof request.body?.entryDate === "string" ? request.body.entryDate.trim() : "";
        const entryNotes = typeof request.body?.notes === "string"
            ? request.body.notes.trim()
            : String(existingAdjustments[0]?.notes ?? "").trim();
        const rawItems = Array.isArray(request.body?.items) ? request.body.items : [];
        if (!Number.isFinite(usdToAwgRate) || usdToAwgRate <= 0) {
            response.status(400).json({ message: "Ingresa una tasa valida en USD@AWG mayor a cero." });
            return;
        ...
        for (const item of items) {
            ...
            await InventoryAdjustment.create({
                ...
                quantity: item.quantity,
                reason: "Entrada de inventario",
                notes: entryNotes || "Registro de entrada editado.",
                entryGroupId: groupId,
                entryWarehouseId: String(warehouse._id),
                entryWarehouseName: String(warehouse.name ?? ""),
                entryUsdToAwgRate: usdToAwgRate,
...
apiRouter.post("/management/inventory-entries/fix-legacy-unit-costs", async (_request, response) => {
    try {
        ...
        for (const adj of legacyAdjustments) {
            ...
            const entryCostUsd = Number(adj.entryCostUsd ?? 0);
            const quantity = Number(adj.quantity ?? 0);
            const entryUsdToAwgRate = Number(adj.entryUsdToAwgRate ?? 0);
        ...
        for (const [productId, data] of latestByProduct.entries()) {
            ...
            await InventoryAdjustment.updateMany({ productId, $or: [{ entryGroupId: { $exists: false } }, { entryGroupId: "" }] }, { $set: { entryCostUsd: unitCostUsd } });
            fixed += 1;
        }
...
apiRouter.put("/management/catalogs/:id/client-pricing", async (request, response) => {
    try {
        ...
        await Promise.all(payload.clientIds.map(async (clientId) => {
            ...
            await CatalogClientPricing.findOneAndUpdate({ catalogId: catalog._id, clientId: client._id }, {
                catalogId: catalog._id,
...
apiRouter.get("/management/logistics-accounting/billed-orders", async (_request, response) => {
    ...
    const billedOrders = await LogisticsInvoice.find({
        ...
        .lean();
    response.json(billedOrders.map((invoice) => ({
...
apiRouter.post("/management/warehouse-locations", async (request, response) => {
    try {
        ...
        const location = await WarehouseLocation.findOneAndUpdate({ warehouseId: warehouse._id, productId: product._id }, {
...
apiRouter.put("/management/import-templates/:id", async (request, response) => {
    try {
        ...
        const template = await ImportTemplate.findOneAndUpdate({ _id: request.params.id, userId }, {
