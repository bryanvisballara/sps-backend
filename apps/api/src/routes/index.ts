...
async function createSubmittedSalesOrder(payload: ReturnType<typeof normalizeSalesOrderPayload>) {
...
async function getClientCatalogProductSalePricesByClientIds(clientIds: string[]) {
  const uniqueClientIds = Array.from(new Set(clientIds.map((clientId) => String(clientId ?? "").trim()).filter(Boolean)));
  const pricingByClientId = new Map<string, Map<string, number>>();
...
async function mapWarehouseOrderRecords(orders: WarehouseOrderDocument[]) {
  if (orders.length === 0) {
    return [];
  }
...
type WarehouseOrderDocument = {
  ...
  invoiceVoidedAt?: unknown;
  invoiceVoidedByUserId?: unknown;
  invoiceVoidedByUserName?: unknown;
  invoiceVoidedByRole?: unknown;
...
function buildWarehouseOrderRecord(
...
async function mapWarehouseOrderRecord(order: WarehouseOrderDocument) {
 ...
 */
async function getClientCatalogProductSalePrices(clientId: string) {
...
async function buildSalesProductCatalog(storeId?: string) {
  const [products, stockByProduct, store, catalogSalePriceByProductId] = await Promise.all([
    ...
    storeId ? getClientCatalogProductSalePrices(storeId) : Promise.resolve(new Map<string, number>()),
  ]);
...
async function enrichInvoiceChangeItems(items: Array<{
  productId: string;
  quantity: number;
  notes?: string;
  description?: string;
...
apiRouter.post("/sales/orders", async (request, response) => {
  try {
    ...
    if (error instanceof Error && "statusCode" in error && typeof (error as { statusCode?: unknown }).statusCode === "number") {
      response.status((error as { statusCode: number }).statusCode).json({ message: error.message });
      return;
    }
...
apiRouter.get("/warehouse/orders", async (request, response) => {
  try {
    ...
    const sort = statuses.length === 1 && statuses[0] === "delivered"
      ? { deliveryDate: -1 as const, createdAt: -1 as const }
      : { deliveryDate: 1 as const, createdAt: 1 as const };
    ...
    response.json({
      orders: await mapWarehouseOrderRecords(orders as WarehouseOrderDocument[]),
      total,
      page,
      pageSize,
    });
...
apiRouter.put("/warehouse/orders/:id/invoice-number", async (request, response) => {
  try {
    ...
    if (updatedOrder.status === "delivered") {
      await CarteraEntry.findOneAndUpdate(
        { orderId: String(updatedOrder._id), active: { $ne: false } },
        { invoiceNumber },
        { runValidators: true },
      );
    }
...
apiRouter.put("/warehouse/orders/:id", async (request, response) => {
  try {
    ...
    const existingItemIds = new Set(
      (Array.isArray(order.items) ? order.items : []).map((item) => String(item.productId)),
    );
    const productsById = new Map(products.map((product) => [String(product._id), product]));
    ...
    if (order.status === "delivered") {
      await applyOrderInventoryDelta(
        String(order._id),
        [...previousItems, ...previousGiftItems],
        [...pricedItems, ...giftItems],
      );
    }
    ...
    if (updatedOrder.status === "delivered") {
      ...
      if (carteraEntry) {
        await updateCarteraAfterInvoiceChange({
          ...
          orderId: String(updatedOrder._id),
          newInvoiceAmountAwg,
          paymentMethod: String(carteraEntry.paymentMethod ?? "credito") as "credito" | "datafono" | "transferencia" | "efectivo",
...
type OrderEditLine = { productId?: unknown; quantity?: unknown; stockRowId?: unknown };

function getOrderEditProductId(item: OrderEditLine) {
  return String(item.productId ?? "").trim();
}

function getOrderEditStockRowId(item: OrderEditLine) {
  return String(item.stockRowId ?? "").trim();
}

function pairOrderItemsForEditLog(previousItems: OrderEditLine[], nextItems: OrderEditLine[]) {
  const previous = previousItems.map((item, index) => ({ item, index }));
  const next = nextItems.map((item, index) => ({ item, index }));
  const usedPrevious = new Set<number>();
  const usedNext = new Set<number>();
  const pairs: Array<{ previous: OrderEditLine; next: OrderEditLine }> = [];

  const tryMatch = (predicate: (previousItem: OrderEditLine, nextItem: OrderEditLine) => boolean) => {
    for (const candidate of next) {
      ...
      const match = previous.find((entry) => (
        !usedPrevious.has(entry.index) && predicate(entry.item, candidate.item)
      ));
  ...
  tryMatch((previousItem, nextItem) => (
    getOrderEditProductId(previousItem) === getOrderEditProductId(nextItem)
...
async function buildWarehouseInvoiceDocumentLines(order: {
  ...
  const regularLines = orderItems.flatMap((item) => {
    ...
    const rate = resolveFrozenOrderItemSalePrice(
      item,
      Number(product?.salePrice ?? 0),
      catalogSalePriceByProductId.get(productId),
    );
...
async function restoreOrderInventoryOnDelete(order: {
  ...
  giftItems?: Array<{ productId?: unknown; quantity?: unknown }>;
}) {
  const orderId = String(order._id);
...
function canVoidDeliveredInvoice(role: string) {
  return role === "management" || role === "contabilidad" || role === "warehouse-aruba";
}
...
async function voidDeliveredInvoiceById(orderId: string, body: Record<string, unknown> = {}) {
  ...
  await LogisticsInvoice.updateMany(
    { orderId: String(order._id), active: { $ne: false } },
    {
      active: false,
      syncExcluded: true,
      notes: voidReason
...
function resolveCarteraPaymentMethodFromStoreDefault(value: unknown): "credito" | "transferencia" | "efectivo" {
...
async function resolveCompletePaymentMethod(rawPaymentMethod: unknown, storeId: unknown) {
...
async function resolveCompleteInvoiceAmountAwg(
  order: {
    storeId?: unknown;
    items?: Array<{ productId?: unknown; quantity?: unknown; salePriceAwg?: unknown }>;
    giftItems?: Array<{ productId?: unknown; quantity?: unknown; stockRowId?: unknown }>;
  },
  rawInvoiceAmount: unknown,
  rawItems: unknown,
) {
...
async function completeWarehouseOrderById(orderId: string, body: Record<string, unknown> = {}) {
  const order = await Order.findById(orderId).lean();
  ...
  if (order.status === "delivered") {
    ...
    return {
      ...
      carteraEntryId: null as string | null,
      invoiceNumber: Number(order.invoiceNumber ?? 0) || null,
  ...
  if (order.status !== "submitted" && order.status !== "dispatched") {
    throw Object.assign(
      new Error("Solo puedes imprimir y facturar pedidos recibidos en bodega."),
      { statusCode: 400 },
    );
  }
...
apiRouter.put("/warehouse/orders/:id/void-invoice", async (request, response) => {
  try {
    ...
    response.json({
      message: result.message,
      order: await mapWarehouseOrderRecord(result.order as Parameters<typeof mapWarehouseOrderRecord>[0]),
      invoiceNumber: result.invoiceNumber,
    });
  } catch (error) {
    if (error instanceof Error && "statusCode" in error && typeof (error as { statusCode?: unknown }).statusCode === "number") {
...
apiRouter.get("/management/catalogs", async (_request, response) => {
  ...
  const assignedClientsByCatalogId = new Map<string, Array<{ clientId: string; clientName: string }>>();
...
apiRouter.put("/management/inventory-entries/:groupId", async (request, response) => {
  try {
    ...
    const aggregateQuantitiesByProduct = (entries: Array<{ productId: string; quantity: number }>) => {
    ...
    const deductWarehouseStock = async (productId: string, warehouseCode: string, quantityToRemove: number) => {
      ...
      const totalAvailable = stockRows.reduce(
        (sum, stockRow) => sum + Number(stockRow.availableUnits ?? 0),
        0,
      );
      ...
      if (totalAvailable < quantityToRemove) {
        throw Object.assign(
          new Error(
            `No se puede reducir el inventario: faltan ${quantityToRemove - totalAvailable} unidad${quantityToRemove - totalAvailable === 1 ? "" : "es"} (ya consumidas). Puedes guardar la nota u observaciones sin bajar cantidades.`,
          ),
          { statusCode: 400 },
        );
      }
    ...
    const creditWarehouseStock = async (params: {
      product: { _id: unknown; inventoryAlert?: number };
      warehouseCode: string;
      quantity: number;
      costUsd: number;
      salePriceAwg: number;
    ...
    const oldQtyByProduct = aggregateQuantitiesByProduct(
    ...
    try {
      ...
      if (
        typeof stockError === "object"
        && stockError !== null
        && "statusCode" in stockError
        && typeof (stockError as { statusCode?: unknown }).statusCode === "number"
      ) {
        response.status((stockError as { statusCode: number }).statusCode).json({
