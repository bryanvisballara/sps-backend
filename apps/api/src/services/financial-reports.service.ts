...
function isDateInRange(dateValue: unknown, startDate: string, endDate: string) {
  if (!dateValue) {
    return false;
  }

  const dateKey = getBusinessDateKey(new Date(dateValue as string | Date));
...
function isDateOnOrBefore(dateValue: unknown, endDate: string) {
...
function getFixedCostAmountForMonth(
  row: { amountAwg?: unknown; frequency?: unknown; startDate?: unknown },
  monthKey: string,
) {
  const startMonth = getBusinessDateKey(new Date(row.startDate as string | Date)).slice(0, 7);
...
function sumFixedCostsForMonths(
  rows: Array<{ amountAwg?: unknown; frequency?: unknown; startDate?: unknown; active?: unknown }>,
  monthKeys: string[],
) {
...
function buildLogisticsCostMap(invoices: Array<{ orderId?: unknown; totalCostAwg?: unknown }>) {
...
function sumCogsForEntries(
  entries: Array<{ orderId?: unknown }>,
  logisticsCostByOrderId: Map<string, number>,
) {
...
export async function buildFinancialReports(params: {
  ...
  const periodEntries = carteraEntries.filter(
    (entry) => matchesStoreFilter(entry, storeId) && isDateInRange(entry.invoicedAt, startDate, endDate),
  );
  const revenue = roundMoney(
    periodEntries.reduce((sum, entry) => sum + Number(entry.invoiceAmountAwg ?? 0), 0),
  );
  ...
  const collectionsInPeriod = roundMoney(
    collections
  ...
  const profitAndLossLines: FinancialReportLine[] = [
  ...
  profitAndLossLines.push(
    { key: "fixed-costs", label: "Costos fijos del periodo", amount: fixedCostsTotal, level: 1 },
    { key: "net-income", label: "Utilidad neta del periodo", amount: netIncome, level: 0, tone: "total" },
    { key: "collections-ref", label: "Recaudo del periodo (referencia)", amount: collectionsInPeriod, level: 1, tone: "reference" },
  );

  let storeBreakdown: FinancialReportsResult["profitAndLoss"]["storeBreakdown"] = null;

  if (!storeId) {
    const storeMap = new Map<string, { storeId: string; storeName: string; revenue: number; costOfGoodsSold: number }>();
  ...
  const receivableEntries = carteraEntries.filter(
    (entry) => matchesStoreFilter(entry, storeId) && isDateOnOrBefore(entry.invoicedAt, endDate),
  );
  const accountsReceivable = roundMoney(
    receivableEntries.reduce((sum, entry) => sum + Number(entry.outstandingAmountAwg ?? 0), 0),
  );

  const cumulativeCollections = roundMoney(
    collections
      .filter((collection) => matchesStoreFilter(collection, storeId) && isDateOnOrBefore(collection.collectedAt, endDate))
      .reduce((sum, collection) => sum + Number(collection.amountAwg ?? 0), 0),
  );

  const cumulativeExpenses = roundMoney(
    expenses
  ...
  const earliestMonth = carteraEntries.reduce((earliest, entry) => {
    const monthKey = getBusinessDateKey(new Date(entry.invoicedAt as string | Date)).slice(0, 7);
  ...
  const inventory = storeId
    ? 0
    : roundMoney(
      warehouseStockRows.reduce(
        (sum, row) => sum + Number(row.availableUnits ?? 0) * Number(row.salePriceAwg ?? 0),
        0,
      ),
    );
  ...
  const cumulativeEntries = carteraEntries.filter(
    (entry) => matchesStoreFilter(entry, storeId) && isDateOnOrBefore(entry.invoicedAt, endDate),
  );
  const cumulativeRevenue = roundMoney(
    cumulativeEntries.reduce((sum, entry) => sum + Number(entry.invoiceAmountAwg ?? 0), 0),
  );
  ...
  const balanceLines: FinancialReportsResult["balance"]["lines"] = [
  ...
  balanceLines.push(
    { key: "total-assets", label: "Total activos", amount: totalAssets, level: 0, tone: "total", section: "assets" },
    { key: "liabilities-header", label: "Pasivos", amount: 0, level: 0, tone: "subtotal", section: "liabilities" },
    { key: "liabilities-none", label: "Sin pasivos registrados", amount: totalLiabilities, level: 1, section: "liabilities" },
    { key: "total-liabilities", label: "Total pasivos", amount: totalLiabilities, level: 0, tone: "subtotal", section: "liabilities" },
    { key: "equity-header", label: "Patrimonio", amount: 0, level: 0, tone: "subtotal", section: "equity" },
    { key: "retained-earnings", label: "Resultados acumulados", amount: totalEquity, level: 1, section: "equity" },
    { key: "total-equity", label: "Total patrimonio", amount: totalEquity, level: 0, tone: "total", section: "equity" },
    {
      key: "balance-check",
  ...
  return {
    ...
    notes,
  } satisfies FinancialReportsResult;
}
