...
export const DEFAULT_QUICKBOOKS_PAYMENT_TERM: QuickBooksPaymentTerm = "Pago a la recepción del servicio";

const QUICKBOOKS_PAYMENT_TERM_SET = new Set<string>(QUICKBOOKS_PAYMENT_TERMS);

const LEGACY_PAYMENT_METHOD_TO_TERM: Record<string, QuickBooksPaymentTerm> = {
...
export function isQuickBooksPaymentTerm(value: string): value is QuickBooksPaymentTerm {
  return QUICKBOOKS_PAYMENT_TERM_SET.has(value);
}

export function normalizeQuickBooksPaymentTerm(value: unknown): QuickBooksPaymentTerm {
...
export function paymentTermToCarteraMethod(term: string): "credito" | "efectivo" {
  const normalized = normalizeQuickBooksPaymentTerm(term);
  return normalized === "Pago a la recepción del servicio" ? "efectivo" : "credito";
}

export function resolveDueDateKeyForPaymentTerm(invoiceDateKey: string, term: string, addDaysToDateKey: (dateKey: string, days: number) => string) {
  const normalized = normalizeQuickBooksPaymentTerm(term);
