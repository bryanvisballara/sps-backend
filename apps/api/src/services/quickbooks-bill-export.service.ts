...
function resolveAdjustmentDateKey(adjustment: { createdAt?: Date | string }) {
...
function resolveGroupSupplier(items: EntryGroup["items"]) {
...
function buildEntryGroups(
  adjustments: Array<{
    ...
    entryWarehouseName?: unknown;
    entryUsdToAwgRate?: unknown;
    entryCostUsd?: unknown;
    createdAt?: Date | string;
    source?: unknown;
  }>,
  productsById: Map<string, { name?: string; sku?: string; supplier?: string; arubaPurchaseCostUsd?: number; arubaUsdToAwgRate?: number }>,
): EntryGroup[] {
...
export async function buildQuickBooksBillExportCsv(params: {
  ...
  const productIds = Array.from(new Set(
    adjustments.map((adjustment) => String(adjustment.productId ?? "")).filter(Boolean),
  ));
